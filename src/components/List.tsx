...
export function Label(props: ILabelProps) {
    return <Flex className="gap-1 my-auto cursor-pointer" click={props.click}>
        <span className="my-auto text_14_400_SFText text-text_primary">{props.name}</span>
...
export function List(props: IListProps) {
    return <ListWrapper>
        ...
        {typeof props.value === "string" ? <span className={`text-right text-white text_14_400_SFText ${props.valueClassName}`}>{props.value}</span> : props.value}
    </ListWrapper>
}
