...
export function TokenName(props: { name: string; className?: string }) {
    return <span title={props.name} className={`text_14_400_SFText truncate text-white leading-none my-auto ${props.className}`}>{props.name}</span>
}
...
export function ListToken(props: ITokenProps) {
    return <Flex className="justify-between cursor-pointer" click={props.select}>
        ...
        <Flex className="!w-fit gap-2">
            <span className={`text_14_400_SFText leading-none text-text_primary my-auto ${props.balance ? "" : "hidden"}`}>{props.balance}{" "}{props.token.name}</span>
